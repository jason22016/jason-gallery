...
1. **优先 Workers Free，但当前尚不能认定免费可部署**。已移除 Paid 的 `cpu_ms:30000`，不默认购买/依赖 Paid。Free 每请求 10 ms CPU / 50 次外部子请求；本地完整认证请求测量和优化后，154 照片的热照片库仍约 18.1 ms（V8 采样，非计费 CPU），冷请求和 40 Project 仍超出目标。先完成 [免费额度评估与后续验收](ADMIN_CPU_PROFILE.md)，不能直接以此版本认定 Free 上线通过。[官方限制](https://developers.cloudflare.com/workers/platform/limits/)。Phase 7 已按用户授权合并 main（`daee9bd`），工作流已支持 `request_id` / `expected_website_commit`；本轮修复单独分支交付。
...
- **缓存**：Worker Cache API 可丢弃地缓存已验证缩略图摘要、ZIP 字节范围/解压文件和已完成任务摘要，最多 1 小时且不超过 artifact 保留期。每个请求先认证；缓存不保存 token、cookie 或签名下载链接，不作为配置/metadata 来源或数据库。GitHub 产物仍受原仓库可见性/保留策略约束，公共仓库 artifact 不是秘密存储。
