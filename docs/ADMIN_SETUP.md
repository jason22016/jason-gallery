...
6. `AUTO_DEPLOY_ENABLED` 保持 `false`：后台保存提交带有 `[skip ci]`，不会触发 push workflow，也不会自动部署。手动 publish 才表示明确发布意图；由于后台提交没有可复用的 main push 测试结论，publish 会运行完整测试、构建和发布流程。普通 main push 若已有同一 commit 的完整成功结果，手动 publish 才可严格复用；无法证明时始终回退到完整测试。定时 workflow 每小时仍同步、测试和构建一次；若想完全由后台发起同步，可另行停用定时调度。
