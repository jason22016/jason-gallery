...
2026-09-11。**新版修复已推送并部署；真实保存已有成功样本，但 state CPU 仍超出 Free 基准，整体验收不通过。** 最新结果见 [部署记录](ADMIN_DEPLOYED_2026-09-11.md)。 旧部署的 1102 / exceededCpu 失败基线仍然有效，不能用本地结果覆盖。验收目标是 154 张照片＋40 个 Project，另有双源同原生 ID 的隔离回归；500 个 Project 是安全上限，不是 Free 性能承诺。
