...
export async function buildRelease(options: { photos: string; root: string; destination: string; websiteCommit: string; runId: string; runNumber: number; production: boolean }) {
  ...
  const build = spawnSync(process.execPath, [path.resolve('node_modules/astro/bin/astro.mjs'), 'build', '--root', path.resolve(options.root), '--outDir', output], { cwd: options.root, stdio: 'inherit', env: { ...process.env, SITE_URL: siteURL ?? '', ASTRO_TELEMETRY_DISABLED: '1' } });
  ...
  const expected = new Set(['index.html', '404.html', 'health.txt', 'favicon.svg', 'sitemap.xml', 'robots.txt', '_headers', 'social/default.jpg', ...projects.flatMap(p => [`projects/${p.slug}/index.html`, ...p.photos.map(x => `projects/${p.slug}/photos/${x.photoId}.json`)]), ...[...ids].map(id => `thumbnails/${id}.jpg`)]);
