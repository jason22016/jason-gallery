...
import { fileHashes, sha256, verifyPhotos } from '../photos/artifact.js';
import { processingFingerprint } from '../photos/fingerprint.js';
import { loadProjectCatalog } from '../../src/projects/loader.js';

export interface Release {
  schemaVersion: 1; websiteCommit: string; photoCommit: string; photoArtifactVersion: string;
...
export async function buildRelease(options: { photos: string; root: string; destination: string; websiteCommit: string; runId: string; runNumber: number; production: boolean }) {
  const photos = await verifyPhotos(options.photos, { fingerprint: await processingFingerprint(), production: options.production });
  ...
  const catalog = loadProjectCatalog({ directory: projectDir, manifestFile: path.join(options.photos, 'photos-manifest.json') });
  ...
  await fs.mkdir(path.join(options.root, 'src/data'), { recursive: true });
  await fs.copyFile(path.join(options.photos, 'photos-manifest.json'), path.join(options.root, 'src/data/photos-manifest.json'));
  await fs.rm(path.join(options.root, 'public/thumbnails'), { recursive: true, force: true });
  await fs.cp(path.join(options.photos, 'public/thumbnails'), path.join(options.root, 'public/thumbnails'), { recursive: true });
  ...
  await verifyPhotos(options.photos, { ref: photos.photoCommit });
  if (sha256(await fs.readFile(path.join(options.root, 'src/data/photos-manifest.json'))) !== photos.files['photos-manifest.json']) throw new Error('Manifest changed during build');
  ...
  const record = { schemaVersion: 1 as const, websiteCommit: options.websiteCommit, photoCommit: photos.photoCommit, photoArtifactVersion: photos.version, projectDigest, fingerprint: photos.fingerprint, runId: options.runId, runNumber: options.runNumber, publishedProjects: projects.length, publicPhotos: ids.size, source: photos.source, files };
  ...
  await fs.writeFile(path.join(output, 'build-version.json'), JSON.stringify({ version: release.version, websiteCommit: release.websiteCommit, photoCommit: release.photoCommit, runNumber: release.runNumber }));
...
export async function verifyRelease(directory: string, production = true) {
  ...
  if (release.schemaVersion !== 1 || sha256(JSON.stringify(record)) !== version || (production && release.source !== 'github')) throw new Error('Invalid release provenance');
  ...
  if (publicVersion.version !== version || publicVersion.websiteCommit !== release.websiteCommit || publicVersion.photoCommit !== release.photoCommit || publicVersion.runNumber !== release.runNumber) throw new Error('Public release version mismatch');
  return release;
}
