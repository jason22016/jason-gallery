...
document.querySelector('#run').onclick=()=>window.spike.run({candidate:document.querySelector('#candidate').value,variant:document.querySelector('#variant').value,limit:86,backend:document.querySelector('#backend').value}).catch(error=>status.textContent=error.stack);
