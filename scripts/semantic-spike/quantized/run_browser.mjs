...
const candidate=args[0]??'v64-int8',backend=args[1]??'webgpu',variant='model';
const label=args[2]??`${candidate}-${backend}`;
...
try {
 ...
 context.on('requestfinished',async request=>{
  ...
  if(response)transfers.push({url:request.url(),status:response.status(),contentLength:Number(response.headers()['content-length']??0),contentEncoding:response.headers()['content-encoding']??null});
 });
 await context.route('**/*',route=>new URL(route.request().url()).origin==='http://127.0.0.1:8769'?route.continue():route.abort());
 ...
 const options={candidate,variant,backend,repeats:profile?1:3,limit:profile?1:86,profile,simulateNoWebGPU:args.includes('--simulate-no-webgpu'),transport:args.includes('--brotli')?'br':'raw'};
