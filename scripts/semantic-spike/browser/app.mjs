...
document.querySelector('#run').onclick=()=>window.spike.run({variant:document.querySelector('#variant').value,backend:document.querySelector('#backend').value}).catch(error=>status.textContent=error.stack);
