...
const out=path.join(root,'.cache/semantic-spike/phase2a');
const args=process.argv.slice(2);
const variant=args[0]??'fp16',backend=args[1]??'webgpu';
const label=args[2]??`${variant}-${backend}`;
...
try {
 ...
 await context.route('**/*',route=>new URL(route.request().url()).origin==='http://127.0.0.1:8767'?route.continue():route.abort());
 ...
 const options={variant,backend,repeats:profile?1:3,limit:profile?1:48,profile,simulateNoWebGPU:args.includes('--simulate-no-webgpu')};
 ...
 result={status:'error',variant,backend,error:error.stack,elapsedMs:Date.now()-started,browserVersion:browser.version()};
