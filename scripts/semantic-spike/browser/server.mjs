...
const cache = path.join(root, '.cache/semantic-spike/phase2a');
const runtime = path.join(cache, 'runtime/node_modules');
const port = Number(process.env.SPIKE_PORT ?? 8767);
...
export const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    else if (/^\/assets\/[a-zA-Z0-9_.-]+\.(json|onnx|f32)$/.test(url.pathname)) file = path.join(cache,url.pathname.slice(1));
