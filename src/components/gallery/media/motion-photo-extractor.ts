 ...
 */
export async function extractMotionPhotoVideo(imageUrl: string, metadata: MotionPhotoMetadata, signal?: AbortSignal): Promise<string | null> {
  try {
    ...
    if (motionPhotoVideoSize && motionPhotoVideoSize > 0) {
      try {
        ...
        if (signal?.aborted) throw rangeError
      }
    }
 ...
 */
async function fetchVideoWithRange(imageUrl: string, offset: number, size: number, signal?: AbortSignal): Promise<Blob | null> {
 ...
 */
async function fetchVideoWithFullDownload(imageUrl: string, offset: number, size?: number, signal?: AbortSignal): Promise<Blob | null> {
  const response = await fetch(imageUrl, { signal })
  if (!response.ok) throw new Error('Motion Photo request failed')
