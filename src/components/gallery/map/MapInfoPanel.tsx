...
export function MapInfoPanel({ collectionTitle, markersCount, bounds }: { collectionTitle: string; markersCount: number; bounds: MapBounds | null }) {
  const [expanded, setExpanded] = useState(false);
  const reduced = useReducedMotion();
  const rangeId = useId();
  const showBounds = expanded && !!bounds;
  return <m.aside className="map-info-panel" aria-label="照片地图信息" initial={reduced ? false : { opacity: 0, x: 20 }}
    animate={{ opacity: 1, x: 0 }} transition={Spring.presets.smooth}>
    <div className="map-info-header">
      <span className="map-info-icon"><Icon name="map-pin" /></span>
      <div className="map-info-summary"><h3><EllipsisWithTooltip>{collectionTitle}</EllipsisWithTooltip></h3>
