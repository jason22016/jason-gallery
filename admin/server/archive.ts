...
export async function archive(github: GitHub, artifact: any) {
  ...
  const reader = new ZipReader(new RangeReader(url.href, github.transport, new ArtifactCache(github.env.GITHUB_REPOSITORY), String(artifact.id), Math.floor((Date.parse(artifact.expires_at)-Date.now())/1000)), { useWebWorkers: false });
  ...
  return { close: () => reader.close(), read: async (name: string) => {
    ...
    if (expanded > 8 * 1024 ** 2) throw new ApiError(413, 'archive', '单次解压内容超过后台 8 MB 上限');
