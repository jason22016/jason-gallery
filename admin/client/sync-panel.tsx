...
import { syncAdditions, type SyncedLibrary } from './sync-additions';
...
export function SyncPanel({ sources, head, connected, dirty, refreshPhotos, pending, setPending, library, addPhotos }: { library: SyncedLibrary; addPhotos: (ids: string[]) => void; sources: PhotoSource[]; head: string; connected: boolean; dirty: boolean; refreshPhotos: () => Promise<boolean>; pending?: { requestId: string; mode: string }; setPending: (value: { requestId: string; mode: string } | undefined) => void }) {
  ...
  const additions = syncAdditions(latest, library);
  ...
  return <section className="sync-panel" aria-label="照片同步面板">
    ...
    <section className="panel sync-result" aria-label="最近完成的同步"><div className="panel-title"><div><h2>最近完成的同步</h2><p>{latest ? `${latest.title} · ${latest.conclusion}${latest.completedAt ? ' · ' + new Date(latest.completedAt).toLocaleString() : ''}${duration !== null ? ' · 耗时 ' + duration + ' 秒' : ''}` : '暂无可验证的同步记录'}</p></div></div>{historyError && <p className="error">{historyError}</p>}{latest?.summary?.sync?.applied && latest?.summary?.adminRead?.status === 'success' ? <><p className="muted">{(latest.summary.sync.sourceIds ?? []).map((id: string) => sources.find(s => s.sourceId === id)?.name ?? id).join('、')} · 已应用到照片库</p><Stats counts={latest.summary.sync.counts} />{!latest.summary.sync.counts && <p className="muted">同步已完成；旧基线不可用，无法确定本次增减数量。</p>}</> : <p className="muted">暂无统计{latest && latest.conclusion !== 'success' ? '，同步未完成或结果尚未确认。' : '，历史任务可能未记录文件差异。'}</p>}{additions && <div className="sync-additions" aria-label="本次同步新增照片">{additions.count === 0 ? <p className="muted">本次同步没有新增照片。</p> : <>{additions.count !== undefined && <p>本次同步新增 {additions.count} 张照片，可整批加入现有 Project 或新建 Project。</p>}{additions.reason && <p className="muted" role="status">{additions.reason}</p>}<button className="primary" disabled={working || !additions.ids.length} onClick={() => addPhotos(additions.ids)}>将新增的 {additions.count ?? '—'} 张照片加入 Project</button><p className="field-hint">加入后可继续编排，点击“保存 Project”后提交。</p></>}</div>}</section>
