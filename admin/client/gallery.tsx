...
import { CoverEditor, CoverPreview } from './cover-editor';
...
export function AdminPreview({ initial, management }: { initial: PreviewData; management?: { saveProof?: string; head: string; email: string; media: any; publishEnabled: boolean } }) {
  ...
  const changeProject = (patch: Partial<Project>) => {
    setProject(p => p && { ...p, ...(patch.coverPhotoId !== undefined && patch.coverPhotoId !== p.coverPhotoId ? { coverCrop: undefined } : {}), ...patch });
    setDirty(true);
  };
  ...
  async function startTask(mode: 'publish') {
    if (busy) return;
    if (management) {
  ...
  return <div className={`admin ${dark ? 'dark' : 'light'}`}>
    ...
    <main aria-busy={busy} inert={busy}>
      ...
      {page === 'projects' && (!project ? <div className="project-cards">{sortedProjects.map(p => <button className="project-card panel" key={p.id} onClick={() => edit(p)}><CoverPreview item={photo(p.coverPhotoId) ?? { sourceId: "", photo: { id: p.coverPhotoId, title: p.title, thumbnailUrl: "", width: 5, height: 6 } }} crop={p.coverCrop} /><div><span>{badge(p.status === 'draft' ? '草稿' : '待网站发布', p.status === 'draft' ? '' : 'green')}</span><h2>{p.title}</h2><p>{p.photos.length} 张照片 · {p.slug}</p><p>展示顺序：{p.order}</p></div><ArrowRight size={20} /></button>)}</div> : <>
        ...
        <div className="editor-layout"><aside className="panel project-form"><div className="panel-title"><h2>Project 信息</h2>{badge(project.status === 'draft' ? '草稿' : '待网站发布')}</div><label>标题<input value={project.title} onChange={e => changeProject({ title: e.target.value })} placeholder="为这组作品起个名字" /></label><label>网址标识 · slug<input disabled={!!management && projects.some(p => p.id === project.id)} value={project.slug} onChange={e => changeProject({ slug: e.target.value })} placeholder="between-places" /></label><label>简短介绍<textarea rows={3} value={project.summary ?? ''} onChange={e => changeProject({ summary: e.target.value })} /></label><label>地点<input value={project.location ?? ''} onChange={e => changeProject({ location: e.target.value })} /></label><label>展示顺序<input type="number" step="any" value={Number.isFinite(project.order) ? project.order : ''} aria-describedby="project-order-hint" onChange={e => changeProject({ order: e.target.valueAsNumber })} /></label><p id="project-order-hint" className="field-hint">数字越小越靠前；相同数字按 slug 排列。保存后需发布网站才会更新主站顺序。</p><label>状态<select aria-label="Project 状态" value={project.status} onChange={e => changeProject({ status: e.target.value as Project['status'] })}><option value="draft">Draft · 草稿</option><option value="published">Published · 下次发布时公开</option></select></label><p className="field-hint">保存 published 状态后，仍需单独发布网站才会上线。</p><div className="form-note"><GitBranch size={16} /><span>只保存 Project 内容与照片引用。<br />照片信息沿用原有产物。</span></div></aside>
          <section className="panel sequence"><div className="panel-title"><div><h2>照片编排 <span className="muted">{project.photos.length}</span></h2><p>第一眼的封面，与之后的观看顺序。</p></div><button className="secondary" onClick={() => { setSelected([]); navigate('photos'); setPhotoTargetId(project.id); setNotice(''); }}><Plus size={15} />添加照片</button></div>
            {photo(project.coverPhotoId) && <div className="project-cover-preview"><CoverPreview item={photo(project.coverPhotoId)!} crop={project.coverCrop} /><div><h3>Project 封面</h3><p className="muted">固定比例 5:6 · 拖动与缩放调整取景</p><button className="secondary" onClick={() => setEditingCover(true)}>调整封面</button></div></div>}
            ...
            <div className="sequence-list">{project.photos.map((p, i) => { const item = photo(p.photoId); return <div className="sequence-row" key={p.photoId}><span className="sequence-number">{String(i + 1).padStart(2, '0')}</span><Thumbnail src={item?.photo.thumbnailUrl} alt={item?.photo.title} /><div className="sequence-info"><strong>{item?.photo.title}</strong><small>{item && sourceName(item.sourceId)}</small>{project.coverPhotoId === p.photoId ? <>{badge('封面', 'green')}<button className="text-button" disabled={!item} onClick={() => setEditingCover(true)}>调整封面裁剪</button></> : <button className="text-button" onClick={() => changeProject({ coverPhotoId: p.photoId })}>设为封面</button>}</div><div className="sequence-controls"><button aria-label={`上移照片 ${i + 1}`} disabled={i === 0} onClick={() => move(i, -1)}><ArrowUp size={15} /></button><button aria-label={`下移照片 ${i + 1}`} disabled={i === project.photos.length - 1} onClick={() => move(i, 1)}><ArrowDown size={15} /></button><button aria-label={`移除照片 ${i + 1}`} onClick={() => { const remaining = project.photos.filter(r => r.photoId !== p.photoId); changeProject({ photos: remaining, coverPhotoId: project.coverPhotoId === p.photoId ? remaining[0]?.photoId ?? '' : project.coverPhotoId }); }}><X size={15} /></button></div></div>; })}</div>
          </section></div></>)}
      ...
      {page === 'tasks' && <><div className="task-actions"><article className="panel"><span className="step">01</span><h2>准备照片库</h2><p>新增照片或来源有变化时，前往照片库预览差异并同步。已有照片可直接用于编排。</p>{badge('按需同步 · 不更新主站')}<button className="secondary" onClick={openSync}>前往照片库 <ArrowRight size={14} /></button></article><article className="panel"><span className="step">02</span><h2>编排并保存 Project</h2><p>选择照片、设置封面和公开状态。照片库可用时，只修改标题、排序或删除 Project，无需重新同步。</p>{badge(management ? '保存后等待发布' : '仅预览内存')}<button className="secondary" onClick={() => navigate('projects')}>管理 Project <ArrowRight size={14} /></button></article><article className="panel"><span className="step">03</span><h2>发布网站</h2><p>确认后更新主站，包含已保存的公开项目和删除操作。草稿不会展示。</p>{badge(management ? (publishEnabled ? '后台手动发布已启用' : '后台发布入口未启用') : '仅界面预览', publishEnabled ? 'green' : 'amber')}<button className="secondary" onClick={() => setPublishConfirm(true)}><CloudUpload size={14} />查看发布步骤</button></article></div>
        ...
        <div className="task-bottom"><span>网站部署：未请求 · 暂无线上 URL</span>{task === 'failure' && <button className="secondary" onClick={() => openSync()}><ArrowRight size={14} />前往照片库</button>}</div></section>}</>}
