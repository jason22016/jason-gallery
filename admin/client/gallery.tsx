...
export function AdminPreview({ initial, management }: { initial: PreviewData; management?: { head: string; email: string; media: any; publishEnabled: boolean } }) {
  ...
  async function saveSource() {
    ...
    if (management) {
      setBusy(true);
      try { const saved = await request('/api/save', { kind: 'sources', expectedHead: head, config: { schemaVersion: 1, sources: next } }); setHead(saved.head); setState('expired'); setSelected([]); setMedia((m: any) => ({ ...m, reason: '来源配置已更新，请同步照片后继续选图' })); }
      catch (e) { fail(e); return; } finally { setBusy(false); }
    }
  ...
  async function refreshPhotos() {
    if (dirty || source) { setError('请先保存或放弃编辑，再刷新仓库。'); return; }
    setBusy(true);
    try { const result = await request('/api/state'); setHead(result.head); setSources(result.sources); setProjects(result.projects); setPhotos(result.media.photos); setMedia(result.media); setState(result.media.state === 'ready' ? 'ready' : result.media.state === 'expired' || result.media.state === 'stale' ? 'expired' : 'failed'); setSelected([]); setProject(null); } catch(e) { fail(e); } finally { setBusy(false); }
  }
  return <div className={`admin ${dark ? 'dark' : 'light'}`}>
    ...
    <main aria-busy={busy}>
      ...
      {page === 'sources' && <><div className="section-line"><span>{sources.filter(s => s.enabled).length} 个启用 · {sources.length} 个来源</span><span><GitBranch size={14} /> GitHub · 只读照片仓库</span></div><div className="source-list">{sources.map(s => <article key={s.sourceId} className={`panel source-card ${!s.enabled ? 'disabled-source' : ''}`}><div className="source-icon"><Layers3 size={22} /></div><div className="source-main"><div className="source-title"><h2>{s.name}</h2>{badge(s.enabled ? '已启用' : '已停用', s.enabled ? 'green' : '')}</div><p>{s.owner} / {s.repo}</p><div className="source-details"><span><GitBranch size={13} />{s.branch}</span><span><FolderOpen size={13} />{s.path || '/ 根目录'}</span><span>{photos.filter(p => p.sourceId === s.sourceId).length} 张照片</span></div></div><div className="source-end"><span className="muted">{projects.filter(p => p.photos.some(r => photo(r.photoId)?.sourceId === s.sourceId)).length} 个 Project 引用</span><button className="secondary" aria-label={`编辑来源 ${s.name}`} onClick={() => { setOriginalSource(s); setSource({ ...s }); setError(''); }}>编辑来源</button></div></article>)}</div><div className="form-note source-note"><CircleAlert size={17} /><p>更换仓库、分支、图片目录或停用来源前，会检查所有 Project 引用。来源身份变化时，已有照片不会被静默替换。</p></div></>}
      {page === 'tasks' && <><div className="task-actions"><article className="panel"><span className="step">01</span><h2>保存内容</h2><p>把照片源配置与 Project 写入网站仓库。</p>{badge(management ? 'GitHub 版本提交' : '仅预览内存')}<button className="secondary" onClick={() => navigate('projects')}>管理 Project <ArrowRight size={14} /></button></article><article className="panel"><span className="step">02</span><h2>同步照片</h2><p>读取启用的照片源，生成完整、可验证的照片产物。</p>{badge('不发布网站')}<button className="secondary" onClick={() => startTask('sync')}><RefreshCw size={14} />同步照片</button></article><article className="panel"><span className="step">03</span><h2>发布网站</h2><p>校验当前内容与照片快照，构建并发布网站。</p>{badge(management?.publishEnabled ? '需要显式发布' : 'Cloudflare 尚未配置', 'amber')}<button className="secondary" onClick={() => setPublishConfirm(true)}><CloudUpload size={14} />查看发布步骤</button></article></div>
