...
- **输入与权限**：网站 main push / 手动 dispatch / 每小时 UTC 第 17 分钟 schedule。照片仓库只读，每次先解析一个 commit，所有 listing/原图 URL 均使用该 commit。保留原生 GitHub provider 扫描，再与该 commit 的 Git tree 交叉核对完整 key/blob/size；tree 截断或缺图失败。PR 和非 main 仅运行无秘密的隔离检查。短期 GITHUB_TOKEN 为网站 Contents Read、Actions Read；可选照片专用 token 仅 Contents Read。Cloudflare Secret 只在发布/回滚步骤注入。
...
- **开关与回滚**：`AUTO_DEPLOY_ENABLED` 默认为 false；push 和 UTC 每小时第 17 分钟的 schedule 仍完成完整并行回归、真实照片和网站构建，配置账号后设 true 自动部署。手动 `mode=publish` 是明确发布指令；当前 commit 若已有成功 main push 且其 `website-release` 与 `execution-summary` 仍完整保留，则复用该次完整回归结论，不重复测试，但仍针对明确选择的照片产物重新构建、执行真实数据审计并验证后发布；无法严格证明时自动回退到全部测试。后台保存提交保留 `[skip ci]`，因此不会触发 push workflow；随后 publish 时因没有同一 commit 的可复用结论，会执行完整测试。回滚前将此变量设 false 防止下次自动前滚，再 dispatch `mode=rollback` 和成功 production deployment UUID。复用 Cloudflare 已保留的完整部署，不依赖本地/Actions 缓存，不混入当前 Project 或照片快照。回滚和发布均在 main workflow 执行。
...
- **同步事务**：所有启用来源先用各自只读 Token（没有则匿名）查询仓库信息并要求 private:false，再解析/验证全部 branch 或指定 commit，从固定 SHA 的完整 tree 逐张匿名读取原图（无认证、禁止重定向）；API 状态与原图可读性分别校验。HTTP/限流/请求 ID 诊断不包含凭据，404 不推断为私有；全部成功后才启动第一个 Builder。每来源独立子进程，绕开上游 workdir 模块常量共享；工作目录和 cache 按 `sourceId/identity` 隔离。默认来源可导入旧原图 blob cache，字节仍逐次验证；派生缓存绑定 fingerprint 和来源身份。每个来源扫描 Contents 并交叉校验固定 Git tree。任一失败不能封存完整 collection 或切换 output；全部照片成功后也必须校验 published/draft 引用才原子切换。成功来源的原图/派生 cache 保留以供重试。
...
- **Git 内容**：网站仓库仍为配置/Project 唯一存储。服务端仅允许配置路径和 `<slug>.json`，严格复用 SourceSchema/ProjectSchema/resolver、固定 legacy alias 与 qualified identity；影响 draft/published 引用的来源变更拒绝。expected head + 单父提交 + 非强制 ref 更新处理竞争；后台保存提交带有 `[skip ci]`，不会触发 main workflow 或隐式发布，明确执行 publish 时再运行完整测试、构建与发布。
