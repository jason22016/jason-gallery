...
| `src/components/gallery/media/useLivePhoto.ts` | `apps/web/src/modules/gallery/MasonryPhotoItem.tsx`, `apps/web/src/lib/image-loader-manager.ts` (`processVideo`, `loadDirectVideo`, `convertVideo`) | Extracts gallery video lifecycle into a hook with a 200ms desktop hover delay, ready/playing/loading/error states, end/leave reset, and lazy local extraction/MOV modules. Adds request aborts, timeout, event/timer/Blob cleanup, rejected-play handling, late-load guards and tab visibility handling. Gates playback, downloads and extraction/MOV work on reduced-motion/mobile conditions, cancels pending work when disabled, and reloads when allowed again. Uses the shared mobile breakpoint. |
...
only its mount-time preference. Snapshot reads use a separate media-query object so
they cannot consume pending change notifications on the subscribed query. The native
indicator branch avoids residual layout projection when reduced motion changes during
a session; live playback and video preparation stop too.
