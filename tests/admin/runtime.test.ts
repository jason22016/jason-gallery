...
test('actual local workerd protects static assets and every write without a signed Access session', {timeout:60000}, async()=>{
  ...
  try{
    ...
    assert(ready,log);
    for(const [p,method] of [['/','GET'],['/api/save','POST'],['/api/delete','POST'],['/api/impact','POST'],['/api/dispatch','POST'],['/api/tasks','GET'],['/api/sync-preview?sourceId=jason-photos','GET'],['/api/thumbnail/1/test','GET']]){const r=await fetch(`http://127.0.0.1:${port}${p}`,{method});assert.equal(r.status,401);assert.equal(r.headers.get('cache-control'),'no-store');assert(!(await r.text()).includes('runtime-test-only-sentinel'));}
