...
test('connected UI sends versioned saves, preserves conflicts, checks impact and distinguishes pending from deployment', async()=>{
  ...
  const state={saveProof:'initial-signed-context',head:'a'.repeat(40),email:'admin@example.com',sources:data.sources,projects:data.projects,publishEnabled:false,media:{state:'ready',photos:data.photos,aliases:{},runId:17,expiresAt:new Date(Date.now()+86400000).toISOString()}};
  ...
  try {
    ...
    let successfulSaves=0; let conflict=true; let failureMode=''; const writes:any[]=[]; let impact=false; let taskData:any={pending:true,tasks:[]};
    await page.route('**/api/**',async route=>{
      const p=new URL(route.request().url()).pathname;
      const body=route.request().method()==='POST'?route.request().postDataJSON():null;
      if(body)writes.push({p,body});
      let response:any=state,status=200;
      if(p==='/api/save' && failureMode==='html') return route.fulfill({status:503,contentType:'text/html',body:'<!doctype html><h1>Error 1102</h1>Worker exceeded resource limits'});
      if(p==='/api/save' && failureMode==='network') return route.abort('failed');
      if(p==='/api/save'){status=conflict?409:200;response=conflict?{message:'仓库已有更新，请合并编辑'}:{status:'saved',head:(++successfulSaves===1?'b':'c').repeat(40),saveProof:'next-signed-context'};}
    ...
    const save=writes.find(v=>v.p==='/api/save');assert.equal(save.body.expectedHead,state.head);assert.equal(save.body.saveProof,state.saveProof);assert.equal(save.body.project.title,'本地编辑不会丢失');
    failureMode='html';await page.getByRole('button',{name:'保存 Project',exact:true}).click();await page.getByRole('alert').filter({hasText:'1102'}).waitFor();assert.equal(await page.getByLabel('标题',{exact:true}).inputValue(),'本地编辑不会丢失');assert(await page.getByText('有未保存修改',{exact:true}).count());
    failureMode='network';await page.getByRole('button',{name:'保存 Project',exact:true}).click();await page.getByRole('alert').filter({hasText:'保存结果尚未确认'}).waitFor();assert.equal(await page.getByLabel('标题',{exact:true}).inputValue(),'本地编辑不会丢失');
    failureMode='';conflict=false;await page.getByRole('button',{name:'保存 Project',exact:true}).click();await page.getByText('Project 已提交 GitHub。网站尚未发布。',{exact:true}).waitFor();
    assert.equal(writes.filter(v=>v.p==='/api/save').length,4,'Failed saves must not retry automatically');
    await page.getByLabel('标题',{exact:true}).fill('第二次保存复用新版本');await page.getByRole('button',{name:'保存 Project',exact:true}).click();
