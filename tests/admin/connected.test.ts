...
test('connected UI sends versioned saves, preserves conflicts, checks impact and distinguishes pending from deployment', async()=>{
  ...
  try {
    ...
    assert.equal(await page.getByText('Cloudflare 尚未配置',{exact:true}).count(),0);
    assert.equal(await page.getByRole('button',{name:'确认发布已保存版本',exact:true}).count(),0);
    await page.getByRole('button',{name:'知道了',exact:true}).click();
    state.publishEnabled=true;
    await page.getByRole('button',{name:'刷新仓库',exact:true}).click();
    await page.getByText('后台手动发布已启用',{exact:true}).waitFor();
    const dispatchCount=writes.filter(v=>v.p==='/api/dispatch').length;
    await page.getByRole('button',{name:'查看发布步骤',exact:true}).click();
    assert.equal(writes.filter(v=>v.p==='/api/dispatch').length,dispatchCount,'Opening confirmation must not publish');
    assert(await page.getByRole('button',{name:'确认发布已保存版本',exact:true}).isEnabled());
    await page.getByRole('button',{name:'确认发布已保存版本',exact:true}).click();
    await page.getByRole('dialog').waitFor({state:'hidden'});
    const publication=writes.filter(v=>v.p==='/api/dispatch').at(-1);
    assert.deepEqual(publication.body,{mode:'publish',expectedHead:state.head,photoRunId:17});
    state.media.state='stale';
    await page.getByRole('button',{name:'刷新仓库',exact:true}).click();
    await page.getByRole('button',{name:'刷新仓库',exact:true}).waitFor({state:'visible'});
