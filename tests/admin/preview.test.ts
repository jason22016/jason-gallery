import test from 'node:test';
import assert from 'node:assert/strict';
import path from 'node:path';
import fs from 'node:fs/promises';
import { chromium, type Browser, type Page } from 'playwright';
import { preparePreview } from './prepare';
import { run, repo } from '../website/fixture';
import { serve } from '../website/server';
import { softwareGPUOptions } from '../browser';

let browser: Browser;
let host: Awaited<ReturnType<typeof serve>>;
const errors: string[] = [];
const projectSnapshot = async () => Object.fromEntries(await Promise.all((await fs.readdir('src/content/projects')).filter(f => f.endsWith('.json')).sort().map(async f => [f, await fs.readFile(path.join('src/content/projects', f), 'utf8')])));
let originalProjects: Awaited<ReturnType<typeof projectSnapshot>>;
test.before(async () => {
  originalProjects = await projectSnapshot();
  await preparePreview();
  run(['node_modules/vite/bin/vite.js', 'build', '--config', 'tests/admin/vite.config.ts'], repo);
  host = await serve(path.resolve('.cache/admin-dist'));
  browser = await chromium.launch(softwareGPUOptions('webgl'));
});
test.after(async () => { await browser?.close(); await host?.close(); });
async function open(width = 1280) {
  const page = await browser.newPage({ viewport: { width, height: 900 } });
  page.on('pageerror', e => errors.push(e.message));
  page.on('request', r => {
    if (!r.url().startsWith(host.url) && !r.url().startsWith('data:') && !(r.url().startsWith('blob:') && new URL(r.url()).origin === new URL(host.url).origin)) errors.push(`Unexpected external request: ${r.url()}`);
    if (r.method() !== 'GET') errors.push(`Unexpected write request: ${r.method()}`);
  });
  await page.goto(host.url);
  await page.getByRole('heading', { name: '每一张，都有它的位置。' }).waitFor();
  return page;
}
const nav = (page: Page, name: string) => page.getByRole('navigation').getByRole('button', { name, exact: true }).click();
async function textVisible(page: Page, text: string) { await page.getByText(text, { exact: false }).first().waitFor(); }

test('cross-source selection, Project validation, cover and ordering remain isolated', async () => {
  const page = await open();
  await page.getByRole('button', { name: '旅行手记 10', exact: true }).click();
  await page.getByLabel('选择 FRAME_002.jpg', { exact: true }).click();
  await page.getByRole('button', { name: '日常观察 8', exact: true }).click();
  await page.getByLabel('选择 FRAME_012.jpg', { exact: true }).click();
  await page.getByRole('button', { name: '加入 Project', exact: true }).click();
  await page.getByRole('button', { name: '新建 Project', exact: true }).click();
  await page.getByRole('button', { name: '保存 Project', exact: true }).click();
  await page.getByRole('alert').waitFor();
  await page.getByLabel('标题', { exact: true }).fill('跨源预览项目');
  await page.getByLabel('网址标识 · slug').fill('fixture-cross-source');
  assert.equal(await page.locator('.sequence-row').count(), 2);
  await page.getByRole('button', { name: '设为封面', exact: true }).click();
  await page.getByLabel('上移照片 2', { exact: true }).click();
  assert.match(await page.locator('.sequence-row').first().innerText(), /FRAME_012/);
  assert.match(await page.locator('.sequence-row').first().innerText(), /封面/);
  await page.getByLabel('Project 状态', { exact: true }).selectOption('published');
  await page.getByRole('button', { name: '保存 Project', exact: true }).click();
  await textVisible(page, '未提交 GitHub，未发布网站');
  await page.getByRole('button', { name: '全部 Project', exact: true }).click();
  await page.getByRole('button', { name: /跨源预览项目/ }).waitFor();
  await page.reload();
  await nav(page, 'Project');
  assert.equal(await page.getByRole('button', { name: /跨源预览项目/ }).count(), 0, 'fixture does not persist formal Projects');
  await page.close();
});

test('source schema, reference impact, new source and disabled source controls', async () => {
  const page = await open();
  await nav(page, '照片源');
  await page.getByLabel('编辑来源 旅行手记').click();
  await page.getByLabel('启用此照片源').uncheck();
  await textVisible(page, '此修改将使以下 Project 的照片引用失效');
  assert.equal(await page.getByRole('button', { name: '保存配置到预览' }).isDisabled(), true);
  await page.getByLabel('启用此照片源').check();
  await page.getByLabel('图片目录', { exact: true }).fill('replacement');
  assert.equal(await page.getByRole('button', { name: '保存配置到预览' }).isDisabled(), true);
  await page.getByRole('button', { name: '取消', exact: true }).click();
  await page.getByRole('button', { name: '新增照片源', exact: true }).click();
  for (const [label, value] of [['显示名称', '测试新来源'], ['稳定来源 ID', 'fixture-new'], ['GitHub 用户 / 组织', 'fixture'], ['仓库名称', 'new-photos']]) await page.getByLabel(label, { exact: true }).fill(value);
  await page.getByLabel('图片目录', { exact: true }).fill('../escape');
  await page.getByRole('button', { name: '保存配置到预览' }).click();
  await page.getByRole('alert').waitFor();
  await page.getByLabel('图片目录', { exact: true }).fill('images');
  await page.getByRole('button', { name: '保存配置到预览' }).click();
  await page.getByRole('heading', { name: '测试新来源' }).waitFor();
  await page.getByLabel('编辑来源 测试新来源').click();
  await page.getByLabel('启用此照片源').uncheck();
  await page.getByRole('button', { name: '保存配置到预览' }).click();
  await textVisible(page, '实际接入后需单独同步照片');
  await page.close();
});

test('missing, expired and failed artifacts offer resync; queued never means published', async () => {
  const page = await open();
  for (const state of ['empty', 'expired', 'failed']) {
    await nav(page, '照片');
    await page.getByLabel('预览产物状态').selectOption(state);
    assert.equal(await page.locator('.photo-card').count(), 0);
    await page.getByRole('button', { name: '前往同步照片', exact: true }).click();
    await page.getByRole('region', { name: '照片同步面板' }).waitFor();
    await page.getByRole('button', { name: '同步照片', exact: true }).click();
    await textVisible(page, '界面预览不能执行同步');
  }
  await nav(page, '发布与记录');
  await page.getByLabel('预览任务状态').selectOption('failure');
  await textVisible(page, 'Source visibility check failed (HTTP 403)');
  await page.getByLabel('预览任务状态').selectOption('success');
  await textVisible(page, '同步完成不代表网站已发布');
  await page.getByRole('button', { name: '查看发布步骤' }).click();
  await page.getByRole('heading', { name: '仅界面预览', exact: true }).waitFor();
  await page.getByRole('button', { name: '知道了' }).click();
  await page.close();
});

test('responsive navigation, dialog keyboard handling and fixture bundle isolation', async () => {
  const page = await open(390);
  for (const name of ['照片', 'Project', '照片源', '发布与记录']) {
    await nav(page, name);
    assert(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), `${name} must not overflow mobile viewport`);
  }
  await nav(page, 'Project');
  await page.getByRole('button', { name: /在路上，慢一点/ }).click();
  assert(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth));
  await nav(page, '照片源');
  await page.getByLabel('编辑来源 旅行手记').click();
  await page.keyboard.press('Escape');
  assert.equal(await page.getByRole('dialog').count(), 0);
  const wasDark = await page.locator('.admin.dark').count() === 1;
  await page.getByRole('button', { name: '切换明暗主题' }).click();
  assert.equal(await page.locator(wasDark ? '.admin.light' : '.admin.dark').count(), 1);
  await page.close();
  assert.deepEqual(errors, []);
  assert.deepEqual(await projectSnapshot(), originalProjects, 'Preview must not change any existing Project file');
  const files = await fs.readdir('.cache/admin-dist/assets');
  const bundle = (await Promise.all(files.filter(f => f.endsWith('.js')).map(f => fs.readFile(path.join('.cache/admin-dist/assets', f), 'utf8')))).join('');
  for (const forbidden of ['JASON_PHOTOS_READ_TOKEN', 'node:fs', 'node:crypto', 'CLOUDFLARE_API_TOKEN']) assert(!bundle.includes(forbidden), forbidden);
});

test('Project filters intersect source/search and bulk selection adds only matching photos', async () => {
  const page = await open();
  const filter = page.getByLabel('按 Project 筛选照片');
  await filter.selectOption('project:fixture-project');
  assert.equal(await page.locator('.photo-card').count(), 4, 'draft membership counts');
  await filter.selectOption('unassigned');
  assert.equal(await page.locator('.photo-card').count(), 14);
  await page.getByRole('button', { name: '旅行手记 10', exact: true }).click();
  assert.equal(await page.locator('.photo-card').count(), 8);
  await page.getByLabel('搜索照片').fill('FRAME_00');
  assert.equal(await page.locator('.photo-card').count(), 7);
  await page.getByRole('button', { name: '全选筛选结果', exact: true }).click();
  assert.equal(await page.locator('.photo-card[aria-pressed=true]').count(), 7);
  await page.getByRole('button', { name: '取消全选筛选结果' }).click();
  assert.equal(await page.locator('.photo-card[aria-pressed=true]').count(), 0);
  await page.getByLabel('搜索照片').fill('missing');
  assert.equal(await page.getByRole('button', { name: '全选筛选结果', exact: true }).isDisabled(), true);
  await page.getByRole('button', { name: '清除筛选' }).click();
  await nav(page, 'Project');
  await page.getByRole('button', { name: /在路上，慢一点/ }).click();
  await page.getByRole('button', { name: '添加照片', exact: true }).click();
  await filter.selectOption('unassigned');
  await page.getByRole('button', { name: '全选筛选结果', exact: true }).click();
  await page.getByRole('button', { name: '加入 在路上，慢一点', exact: true }).click();
  assert.equal(await page.locator('.sequence-row').count(), 18);
  await page.getByRole('button', { name: '名称倒序', exact: true }).click();
  assert.match(await page.locator('.sequence-row').first().innerText(), /FRAME_018/);
  assert.match(await page.locator('.sequence-row').last().innerText(), /FRAME_001.*封面/s);
  await page.getByRole('button', { name: '名称正序', exact: true }).click();
  assert.match(await page.locator('.sequence-row').first().innerText(), /FRAME_001.*封面/s);
  await page.getByRole('button', { name: '添加照片', exact: true }).click();
  assert.equal(await page.locator('.photo-card').count(), 0, 'unsaved additions are already assigned');
  await page.getByRole('button', { name: '返回 在路上，慢一点', exact: true }).click();
  await page.getByRole('button', { name: '保存 Project', exact: true }).click();
  await page.getByRole('button', { name: '全部 Project', exact: true }).click();
  await page.getByRole('button', { name: /在路上，慢一点/ }).click();
  assert.equal(await page.locator('.sequence-row').count(), 18);
  assert.match(await page.locator('.sequence-row').last().innerText(), /FRAME_018/);
  await page.close();
});

test('admin follows device appearance on first load and live changes, including mobile controls', async () => {
  const page = await open(390);
  await page.emulateMedia({ colorScheme: 'light' });
  await page.reload();
  await page.locator('.admin.light').waitFor();
  assert.equal(await page.locator('.admin').evaluate(el => getComputedStyle(el).colorScheme), 'light');
  await page.getByLabel('按 Project 筛选照片').selectOption('unassigned');
  await page.getByRole('button', { name: '全选筛选结果', exact: true }).click();
  await page.screenshot({ path: '.cache/admin-filters-light-mobile.png', fullPage: true });
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
  await page.emulateMedia({ colorScheme: 'dark' });
  await page.locator('.admin.dark').waitFor();
  await page.reload();
  await page.locator('.admin.dark').waitFor();
  await page.getByLabel('切换明暗主题').click();
  await page.locator('.admin.light').waitFor();
  await page.close();
});

test('compact list and image views share filters and selection on desktop and mobile', async () => {
  for (const width of [1280, 390]) {
    const page = await open(width);
    await page.emulateMedia({ colorScheme: 'light' });
    const views = page.getByRole('group', { name: '照片显示方式' });
    await page.getByLabel('按 Project 筛选照片').selectOption('unassigned');
    await page.getByLabel('搜索照片').fill('FRAME_01');
    const names = await page.locator('.photo-card').evaluateAll(cards => cards.map(card => card.getAttribute('aria-label')));
    await page.locator('.photo-card').first().click();
    await views.getByRole('button', { name: '列表', exact: true }).click();
    assert.equal(await views.getByRole('button', { name: '列表', exact: true }).getAttribute('aria-pressed'), 'true');
    assert.deepEqual(await page.locator('.photo-table .photo-card').evaluateAll(cards => cards.map(card => card.getAttribute('aria-label'))), names);
    assert.equal(await page.locator('.photo-table .photo-card[aria-pressed=true]').count(), 1);
    const row = page.locator('.photo-table .photo-card').first();
    assert((await row.boundingBox())!.height < 90, 'List rows must remain compact');
    assert((await row.locator('img').boundingBox())!.width <= 64, 'List uses small thumbnails');
    await page.waitForFunction(() => [...document.querySelectorAll('.photo-table .photo-info')].every(el => getComputedStyle(el).opacity === '1'));
    assert.equal(await row.locator('.photo-info').evaluate(el => getComputedStyle(el).opacity), '1', 'Names remain visible without hovering');
    await row.focus(); await page.keyboard.press('Space');
    assert.equal(await page.locator('.photo-card[aria-pressed=true]').count(), 0);
    await page.getByRole('button', { name: '全选筛选结果', exact: true }).click();
    assert.equal(await page.locator('.photo-card[aria-pressed=true]').count(), names.length);
    await views.getByRole('button', { name: '图像', exact: true }).click();
    assert.equal(await page.locator('.photo-grid .photo-card[aria-pressed=true]').count(), names.length);
    await views.getByRole('button', { name: '列表', exact: true }).click();
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
    await page.screenshot({ path: `.cache/admin-list-${width}.png` });
    await page.getByRole('button', { name: '加入 Project', exact: true }).click();
    await page.getByRole('dialog').getByRole('button', { name: /在路上，慢一点/ }).click();
    assert.equal(await page.locator('.sequence-row').count(), 4 + names.length);
    await page.getByRole('button', { name: '添加照片', exact: true }).click();
    assert.equal(await views.getByRole('button', { name: '列表', exact: true }).getAttribute('aria-pressed'), 'true', 'Picker retains view mode');
    assert.equal(await page.locator('.photo-card').count(), 0, 'Draft additions update the unassigned filter in list mode');
    await page.close();
  }
});
